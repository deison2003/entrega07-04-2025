...
-- 1. Modificar tabla auditoria (si no existe el campo y FK)
...
-- 2. Agregar campo direccion_ip a pedidos
ALTER TABLE pedidos
ADD COLUMN direccion_ip VARCHAR(45);

-- 3. Crear la vista vista_auditoria_resumen
...
-- 4. Crear trigger para auditoria de productos en actualización de stock
DELIMITER $$
...
BEGIN
    IF OLD.stock != NEW.stock THEN
        INSERT INTO auditoria (accion, tabla_afectada, fecha, id_usuario, direccion_ip)
        VALUES (CONCAT('Stock actualizado de ', OLD.stock, ' a ', NEW.stock),
                'productos',
                NOW(),
                NULL,
                NULL);
